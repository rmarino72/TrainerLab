...
{
    [JsonIgnore] protected Logger Logger { set; get; }
    ...
    {
        var message = string.IsNullOrEmpty(stackTrace) ? msg : $"{msg} - {stackTrace}";
    ...
    {
        var message = string.IsNullOrEmpty(ex.StackTrace) ? ex.Message : $"{ex.Message} - {ex.StackTrace}";
        if (Logger != null) Logger.Error(FormatLogMsg(message, method));
        Console.WriteLine(FormatLogMsg(message, method));
    }
    ...
    {
        return (BasicObject)MemberwiseClone();
    }
}