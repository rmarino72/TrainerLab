...
public class SchedulerBl : GenericBl
{
    ...
    public RestObjectResult NewSlot(Slot slot)
    {
        try
        {
            using (var transactionScope = BODB.CreateTransactionScope())
            {
    ...
    public RestObjectResult UpdateSlot(Slot slot)
    {
        try
        {
            using (var transactionScope = BODB.CreateTransactionScope())
            {
                var slotTmp = BODB.GetSlotById(slot.Id);
    ...
    public RestObjectResult DeleteSlot(int id)
    {
        try
        {
            var slot = BODB.GetSlotById(id);
            if (slot == null) return MakeRestObjectResponse(null, false, 3, "Slot not found");
            if (slot.Status == "DONE") return MakeRestObjectResponse(null, false, 2, "Can't delete a done Slot");
            BODB.DeleteSlot(id);
            return MakeRestObjectResponse(null);
        }
        catch (Exception ex)
    ...
    public RestListResult GetMySlots(string email, DateTimeInterval interval)
    {
        try
        {
            var slots = BODB.GetSlotByInterval(interval);
            return MakeRestListResponse(slots.FindAll(x => x.Email == email || x.Status == SlotStatusEnum.FREE)
                .Cast<BasicObject>().ToList());
        }
        catch (Exception ex)
        {
            return HandleListException(ex);
    ...
    public RestObjectResult FreeSlot(int id)
    {
        try
        {
            ...
            {
                var slot = BODB.GetSlotById(id);
                if (slot.StartDateTime < DateTime.Now.AddHours(Config.DismissTolerance) &&
                    slot.Status == SlotStatusEnum.CONFIRMED) return MakeRestObjectResponse(null, false, 1, "Too late");
    ...
    public RestListResult GetSlotsByStatus(string status)
    {
        try
        {
            var interval = new DateTimeInterval { Start = DateTime.Now, End = DateTime.Now.AddYears(100) };
            var slots = BODB.GetSlotByInterval(interval);
